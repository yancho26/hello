...
import { analyseMeasurements, growthConcerns, midParentalHeight } from '../public/js/shared/growth.js';
import { defaultSchedule } from '../public/js/shared/calendar.js';
import {
  CHECKPOINT_BY_AGE, SCREENING_RESULTS, SCREENING_TOOLS, developmentSummary,
...
function patientCard(store, p, opts) {
  ...
  const concerns = (p.measurements || []).length
    ? growthConcerns(p, analyseMeasurements(p, p.measurements)) : [];
  const devSummary = developmentSummary(p, p.development, ageInMonthsExact(p.birthDate, today()));
  const devConcerns = devSummary.concerns.filter(c => c.severity >= 2);
  return {
    id: p.id,
    name: p.name,
    egn: p.egn || '',
    ...
    counts: s.counts,
    coverage: s.coverage,
    growthConcerns: concerns.filter(c => c.severity >= 2).length,
    developmentConcerns: devConcerns.length,
    developmentDue: devSummary.dueCheckpoint && !devSummary.dueDone ? devSummary.dueCheckpoint.ageMonths : null,
    needsAttention: s.needsAttention || concerns.some(c => c.severity >= 2) || devConcerns.length > 0,
    next: s.next ? {
...
export const routes = {
  ...
  'GET /api/patients/:id': (ctx) => {
    ...
    return {
      ...
      growth,
      concerns: growthConcerns(p, growth),
      development: developmentSummary(p, p.development, ageInMonthsExact(p.birthDate, today())),
      targetHeight: p.parentHeights
        ? midParentalHeight(p.sex, p.parentHeights.mother, p.parentHeights.father) : null,
    };
  },
  ...
  'POST /api/patients/:id/measurements': (ctx) => {
    ...
    const m = {
      ...
      systolic: num(body.systolic, 'Систолно налягане', { min: 50, max: 250 }),
      diastolic: num(body.diastolic, 'Диастолно налягане', { min: 20, max: 160 }),
    ...
    if ((m.systolic === null) !== (m.diastolic === null)) {
      bad('Въведете и двете стойности на артериалното налягане.');
    }
    if (m.systolic !== null && m.diastolic >= m.systolic) {
      bad('Диастолното налягане трябва да е под систолното.');
    }
    ...
    const growth = analyseMeasurements(p, p.measurements);
    return { measurement: m, growth, concerns: growthConcerns(p, growth) };
  },
  ...
  'POST /api/patients/:id/development': (ctx) => {
    const { store, params, body, doctor } = ctx;
    const p = store.patient(params.id);
    if (!p) notFound('Досието не е намерено.');

    const checkpoint = num(body.checkpoint, 'Контролна възраст', { min: 0, max: 240, required: true });
    const cp = CHECKPOINT_BY_AGE.get(checkpoint);
    if (!cp) bad('Няма контролен лист за тази възраст.');

    const allowed = new Set(cp.items.map(i => i.id));
    const answers = {};
    for (const [key, value] of Object.entries(body.answers || {})) {
      if (!allowed.has(key)) continue;
      if (!['yes', 'not_yet', 'unsure'].includes(value)) continue;
      answers[key] = value;
    }

    let screening = null;
    if (body.screening && body.screening.tool) {
      const tool = str(body.screening.tool, 'Инструмент', { max: 40 });
      if (!SCREENING_TOOLS.some(t => t.id === tool)) bad('Непознат скринингов инструмент.');
      const result = str(body.screening.result, 'Резултат', { required: true, max: 20 });
      if (!SCREENING_RESULTS[result]) bad('Непознат резултат от скрининга.');
      screening = {
        tool,
        result,
        score: str(body.screening.score, 'Точки', { max: 40 }),
        note: str(body.screening.note, 'Бележка', { max: 500 }),
      };
    }
    ...
    const rec = {
      ...
      answers,
      screening,
      lostSkills: !!body.lostSkills,
      parentConcern: !!body.parentConcern,
      action: str(body.action, 'Предприето', { max: 500 }),
      note: str(body.note, 'Бележка', { max: 2000 }),
      doctorId: doctor ? doctor.id : '',
      recordedAt: new Date().toISOString(),
    };
    if (rec.date > today()) bad('Датата не може да е в бъдещето.');
    if (rec.date < p.birthDate) bad('Датата е преди раждането на детето.');

    p.development = (p.development || []).filter(r => r.checkpoint !== checkpoint);
    p.development.push(rec);

    /* Записаният скрининг затваря и съответната задача в календара.
     * Наблюдението по контролния лист само по себе си НЕ я затваря:
     * попълненият списък с етапи е surveillance, а задачата иска
     * стандартизиран скрининг — това са различни неща. */
    const closed = [];
    if (screening) {
      const tool = SCREENING_TOOLS.find(t => t.id === screening.tool);
      const kind = tool ? tool.kind : 'general';
      const ids = kind === 'autism' ? [`asd-screen-${checkpoint}`] : [`dev-screen-${checkpoint}`];
      for (const id of ids) {
        if (!store.schedule.some(i => i.id === id)) continue;
        if (p.records[id] && p.records[id].status === 'done') continue;
        p.records[id] = {
          status: 'done',
          date: rec.date,
          batch: '', product: '',
          reason: '', deferUntil: '',
          note: `Отбелязано автоматично при запис на оценка на развитието (${tool ? tool.label : screening.tool}).`,
    ...
    p.updatedAt = new Date().toISOString();
    store.persist();
    store.audit(doctor, 'development_add',
      { patientId: p.id, name: p.name, checkpoint, date: rec.date, lostSkills: rec.lostSkills });
    return {
      record: rec,
      closedItems: closed,
      development: developmentSummary(p, p.development, ageInMonthsExact(p.birthDate, today())),
  ...
  'DELETE /api/patients/:id/development/:devId': (ctx) => {
    const { store, params, doctor } = ctx;
    const p = store.patient(params.id);
    if (!p) notFound('Досието не е намерено.');
    const before = (p.development || []).length;
    p.development = (p.development || []).filter(r => r.id !== params.devId);
    if (p.development.length === before) notFound('Записът не е намерен.');
    store.persist();
    store.audit(doctor, 'development_delete', { patientId: p.id, name: p.name, recordId: params.devId });
    return {
      development: developmentSummary(p, p.development, ageInMonthsExact(p.birthDate, today())),
    };
  },
...
function readPatientFields(body, isNew) {
  ...
  if (has('parentHeights')) {
    const ph = body.parentHeights || {};
    out.parentHeights = {
      mother: num(ph.mother, 'Ръст на майката', { min: 120, max: 220 }),
      father: num(ph.father, 'Ръст на бащата', { min: 120, max: 220 }),
    };
  }
  if (has('birth')) {
    const b = body.birth || {};
